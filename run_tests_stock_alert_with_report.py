...
def parse_test_results(output_lines, module_name):
    ...
    stats = {
        ...
        'executed_tests': []  # Lista para almacenar tests
    }
    
    # Regex patterns - adaptados para el módulo
...
def generate_pdf_report(filename, stats, module_name):
    ...
    elements.append(Paragraph("Módulo: Alertas de Stock Crítico", styles['Heading2']))
    ...
    elements.append(Paragraph(f"<b>Módulo Técnico:</b> {module_name}", info_style))
...
if __name__ == "__main__":
    MODULE = "stock_critical_alert"
    DB = "test_stock_alert_clean"
    PORT = 8097
