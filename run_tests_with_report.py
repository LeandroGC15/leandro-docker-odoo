...
def parse_test_results(output_lines):
    ...
    stats = {
        ...
        'executed_tests': []  # Nueva lista para almacenar tests
    }
    
    # Regex patterns
    summary_pattern = re.compile(r"modulos_contabilidad: (\d+) tests (\d+\.\d+)s")
    test_start_pattern = re.compile(r"Starting (Test\w+)\.(test_\w+)")
    module_load_fail = re.compile(r"Module modulos_contabilidad: (\d+) failures, (\d+) errors")
...
def generate_pdf_report(filename, stats, module_name):
    ...
    elements.append(Paragraph(f"<b>Módulo:</b> {module_name}", info_style))
    ...
    if stats['executed_tests']:
        ...
        # Agrupamos por Clase de Test para mejor legibilidad (opcional, aquí listamos directo)
...
if __name__ == "__main__":
    MODULE = "modulos_contabilidad"
    DB = "test_contabilidad_clean"
    PORT = 8099
    ...
    statistics = parse_test_results(logs)
    
    # 3. Generar PDF
    pdf_filename = f"/home/odoo/qa_report_{MODULE}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    try:
        generate_pdf_report(pdf_filename, statistics, MODULE)
